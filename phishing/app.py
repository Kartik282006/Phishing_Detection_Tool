...
PostgreSQL-backed for production deployment on Render.
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'super_secure_secret_key_change_me')
...
# ── Database URL ─────────────────────────────────────────────────────
# Render injects DATABASE_URL automatically when a PostgreSQL service is linked.
# Render still uses the legacy "postgres://" scheme — psycopg2 needs "postgresql://".
...
def insert_scan(url, score, level):
    ...
    c.execute(
        'INSERT INTO scans (url, score, level, timestamp) VALUES (%s, %s, %s, %s)',
...
def get_recent_scans(limit=10):
    ...
    c = conn.cursor()
    c.execute(
        'SELECT id, url, score, level, timestamp FROM scans ORDER BY id DESC LIMIT %s',
        (limit,)
    )
    rows = c.fetchall()
    conn.close()
...
def chat():
    ...
    if "scan" in lower_message:
        ...
        if urls:
            ...
            formatted_response = (
                f"I scanned {spoken_domain}. The risk score is {result['score']}, "
                f"and the threat level is marked as {result['level']}."
            )
    ...
    if is_command_handled:
        ...
        response_data = {"response": formatted_response, "type": "analysis"}

    else:
        response_text = llm_engine.ask_llm(user_message, session['history'])
        response_data = {"response": response_text, "type": "qa"}
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO chats (user_message, bot_response, response_type) VALUES (%s, %s, %s)",
...
# ── Startup ───────────────────────────────────────────────────────────
# Called at module level so gunicorn workers initialise the DB on startup.
try:
    init_db()
except Exception as e:
    print(f"Warning: DB initialisation failed — {e}")
